...
class WeaponEditor:
    ...
    def create_widgets(self):
        ...
        # Список оружия
        self.weapon_list = tk.Listbox(left_frame, width=35, height=40)
    ...
    def create_editor_form(self, parent):
        ...
        self.name_entry = ttk.Entry(form_frame, width=70)
        ...
        self.categories_entry = ttk.Entry(form_frame, width=70)
        self.categories_entry.grid(row=1, column=1, sticky=tk.EW, pady=2)
        
        # Описание
        ttk.Label(form_frame, text="Описание:").grid(row=2, column=0, sticky=tk.NW)
        self.description_text = tk.Text(form_frame, width=70, height=12, wrap=tk.WORD, font=('Arial', 10))
        self.description_text.grid(row=2, column=1, sticky=tk.EW, pady=2)
        
        # Характеристики (xaract)
        ttk.Label(form_frame, text="Характеристики:").grid(row=3, column=0, sticky=tk.NW)
        self.xaract_text = tk.Text(form_frame, width=70, height=12, wrap=tk.WORD, font=('Arial', 10))
        self.xaract_text.grid(row=3, column=1, sticky=tk.EW, pady=2)
        
        # Синие статы
        ttk.Label(form_frame, text="Синие статы:").grid(row=4, column=0, sticky=tk.NW)
        self.blue_stat_text = tk.Text(form_frame, width=70, height=12, wrap=tk.WORD, font=('Arial', 10))
    ...
    def display_weapon(self, index):
        if 0 <= index < len(self.data["weapon"]):
            ...
            self.description_text.insert(tk.END, weapon.get("description", ""))
    ...
    def save_changes(self):
        if 0 <= self.current_index < len(self.data["weapon"]):
            ...
            weapon["description"] = self.description_text.get(1.0, tk.END).strip()
